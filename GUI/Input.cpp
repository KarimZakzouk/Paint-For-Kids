#include "Input.h"
#include "Output.h"
const double PI = 3.14159265358979323846; // or use constexpr
...
void Input::CheckPointClicked(int CaseShape, Output* outPtr, Point& P1, Point* P2) const
{
	...
	case ITM_HEX:
	{
		while (P1.y - outPtr->GetLengthHex() * cos((PI / 6)) < UI.ToolBarHeight ||
			UI.height - (P1.y + outPtr->GetLengthHex() * cos((PI / 6)))<UI.StatusBarHeight||
			P1.x - outPtr->GetLengthHex() * cos((PI / 6)) < 0 || (P1.x + outPtr->GetLengthHex() * cos((PI / 6))) > UI.width-15)		//Hexagon validity
		{
			outPtr->ClearStatusBar();
